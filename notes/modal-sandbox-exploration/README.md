...
Modal's TypeScript SDK allows creating sandboxes - secure, isolated containers
that boot in seconds. Useful for running arbitrary code, custom daemons, or
...
Requires Node 22+ (or Bun). Auth via `MODAL_TOKEN_ID` and `MODAL_TOKEN_SECRET`
env vars.
...
Note: Internal process activity (CPU work, background jobs) does NOT count as
activity.
